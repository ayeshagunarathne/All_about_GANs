...
class Discriminator(nn.Module):
    def __init__(self, in_features=784, hidden_dim=64, out_features=1):
        super().__init__()
...
class Generator(nn.Module):
    def __init__(self, z_dim = 10, in_features=784, hidden_dim=64):
...
for epoch in range(num_epochs):
    for real, _ in tqdm(dataloader):  #we dont use labels so we use _ here
        ...
        #Train Discriminator
        Noise_vector = torch.randn(current_batch_size, z_dim).to(device)
        ...
        if current_step % display_step == 0 and current_step > 0:
            ...
            fake_noise = torch.randn(current_batch_size, z_dim).to(device)
            fake = gen(fake_noise)
